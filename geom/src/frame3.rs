use crate::{axis3::Axis3, pt3::Pt3, quat::Quat, scalar::Scalar, vec3::Vec3};
use std::{
    fmt,
...
impl<T: Scalar> Frame3<T> {
    ...
    fn validate_orthonormal_right_handed(self) {
        ...
        assert!(!(handedness < T::ZERO), "frame is not right-handed!");

        assert!(
            !((handedness - T::ONE) > T::from_f64(HANDEDNESS_EPS)),
...
impl<T: Scalar> Display for Frame3<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "position: {}\nlocal x: {}\nlocal y: {}\nlocal z: {}",
            self.origin, self.lx, self.ly, self.lz
        )
    }
}
...
mod tests {
    ...
    scalar_test!(test_from_xy_basic, |T| {
        ...
        let frm = Frame3::<T>::from_xy(pos, lx, ly);

        assert_eq!(frm.origin, pos);
    ...
    scalar_test!(test_from_xz_basic, |T| {
        ...
        let frm = Frame3::<T>::from_xz(pos, lx, lz);
    ...
    scalar_test!(test_from_yz_basic, |T| {
        ...
        let frm = Frame3::<T>::from_yz(pos, ly, lz);
    ...
    scalar_test!(test_from_xyz_basic, |T| {
        ...
        let frm = Frame3::<T>::from_xyz(pos, lx, ly, lz);
    ...
    scalar_test!(test_identity, |T| {
        let identity = Frame3::<T>::IDENTITY;
        let expected = Frame3 {
            origin: Pt3::ZERO,
            lx: Vec3::<T>::UNIT_X,
            ly: Vec3::<T>::UNIT_Y,
    ...
    fn test_translate<T: Scalar>(frm: Frame3<T>, v: Vec3<T>) {
        ...
        assert_eq!(translated.origin(), expected.origin());
